# -*- coding: utf-8 -*-
"""Phishing_URL_Detection_ngp.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1LAzqIHB2Pun3lxjk0sf_ESYQDv0ZKgyp
...
# Now, fit the XGBoost model using the encoded target variable
xgb.fit(X_train, y_train_encoded)

# For predictions, you'll need to inverse transform the predictions
# to get back to the original labels (-1 and 1)
...
# Predictions on training data using encoded labels
y_train_pred_encoded = xgb.predict(X_train)
# Convert encoded predictions back to original labels for training data
y_train_pred = le.inverse_transform(y_train_pred_encoded)

# Predictions on test data using encoded labels (already done in previous step)
# y_pred_encoded = xgb.predict(X_test)
# Convert encoded predictions back to original labels for test data (already done in previous step)
# y_pred = le.inverse_transform(y_pred_encoded)

acc_train_xgb = metrics.accuracy_score(y_train, y_train_pred)
acc_test_xgb = metrics.accuracy_score(y_test, y_pred)
print("XGBoost Classifier : Accuracy on training Data: {:.3f}".format(acc_train_xgb))
print("XGBoost Classifier : Accuracy on test Data: {:.3f}".format(acc_test_xgb))
print()

f1_score_train_xgb = metrics.f1_score(y_train, y_train_pred, average='weighted')
f1_score_test_xgb = metrics.f1_score(y_test, y_pred, average='weighted')
print("XGBoost Classifier : f1_score on training Data: {:.3f}".format(f1_score_train_xgb))
print("XGBoost Classifier : f1_score on test Data: {:.3f}".format(f1_score_test_xgb))
print()

recall_score_train_xgb = metrics.recall_score(y_train, y_train_pred)
recall_score_test_xgb = metrics.recall_score(y_test, y_pred)
print("XGBoost Classifier : Recall on training Data: {:.3f}".format(recall_score_train_xgb))
print("XGBoost Classifier : Recall on test Data: {:.3f}".format(recall_score_test_xgb))
print()

precision_score_train_xgb = metrics.precision_score(y_train, y_train_pred)
precision_score_test_xgb = metrics.precision_score(y_test, y_pred)
...
# Advanced ML Algorithm: Stacking Classifier
...
# Define the meta learner; here we reuse the GradientBoostingClassifier
meta_estimator = GradientBoostingClassifier(max_depth=4, learning_rate=0.7, random_state=42)
...
accuracy_stack = metrics.accuracy_score(y_test, y_pred_stack)
f1_stack = metrics.f1_score(y_test, y_pred_stack, average='macro')

print("Stacking Classifier Accuracy:", accuracy_stack)
print("Stacking Classifier F1 Score:", f1_stack)

# Save the trained stacking model to a pickle file
pickle.dump(stacking_clf, open('stacking_model.pkl', 'wb'))
...
recall_stack = metrics.recall_score(y_test, y_pred_stack)
precision_stack = metrics.precision_score(y_test, y_pred_stack)
...
# Store the stacking classifier performance results using the existing storeResults function.
storeResults('Stacking Classifier', accuracy_stack, f1_stack, recall_stack, precision_stack)
...
## 5.12. Advanced Stacked Classifier (GBM+SVM+RF)
from sklearn.ensemble import StackingClassifier
from sklearn.linear_model import LogisticRegression

# First-line models
base_models = [
    ('GradientBoosting', GradientBoostingClassifier(max_depth=4, learning_rate=0.7, random_state=42)),
...
y_test_stacked = stacked_classifier.predict(X_test)

# Store metrics
acc_stacked = metrics.accuracy_score(y_test, y_test_stacked)
f1_stacked = metrics.f1_score(y_test, y_test_stacked)
storeResults('Stacked Ensemble (GBM+SVM+RF)', acc_stacked, f1_stacked,
            metrics.recall_score(y_test, y_test_stacked),
            metrics.precision_score(y_test, y_test_stacked))

# Calculate and print accuracy
acc_stacked = metrics.accuracy_score(y_test, y_test_stacked)
print(f"Stacked Ensemble Accuracy: {acc_stacked}")

# Store metrics (Assuming you have a storeResults function)
# storeResults('Stacked Ensemble (GBM+SVM+RF)', acc_stacked, f1_stacked, ...)

# Save the stacked model
pickle.dump(stacked_classifier, open('p_stacked_model.pkl', 'wb'))
print("Stacked_stacked_model.pkl")
